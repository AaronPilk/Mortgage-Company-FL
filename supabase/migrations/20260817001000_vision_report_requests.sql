...
begin
  ...
  insert into public.vision_assumptions (
    project_id, assumption_key, value, unit, source_kind,
    source_reference, confirmed_by_user
  )
  select
    v_project_id,
    assumption.key,
    ...
    case
      when assumption.value->>'source' = 'user' then 'user'
      else 'company_default'
    end,
    case
      when assumption.value->>'source' = 'user' then 'visitor_report_request'
      else p_scenario->>'calculation_version'
    end,
    assumption.value->>'source' = 'user'
