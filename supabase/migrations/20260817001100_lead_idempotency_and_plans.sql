...
-- Keep the planner's public contract while moving its receipt write onto the
-- exact-retry function above. The legacy five-argument receipt function remains
-- only for migration compatibility and is no longer executable by service_role.
...
declare
  v_lead_id uuid;
begin
  v_lead_id := public.create_lead_with_receipt(
    p_lead,
    p_consent,
    p_attribution,
    p_outbox,
...
revoke execute on function public.create_lead_with_receipt(jsonb, jsonb, jsonb, jsonb, uuid) from service_role;
