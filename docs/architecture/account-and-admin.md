...
The account migrations are local and additive. They have not been applied to any remote project because the visible Supabase project has not been proven to belong to TRACT. Cloudflare remains the only application-hosting architecture; Vercel remains read-only and outside the release path.
