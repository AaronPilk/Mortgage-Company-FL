...
export function RefinanceCalculator() {
  ...
  return (
    <>
          ...
          <label className="mt-5 flex items-start gap-3 text-sm">
            ...
            <span>
              ...
              <span className="mt-1 block text-xs text-[var(--text-muted)]">
                This does not make the costs disappear. It moves them into the balance, where you
                pay interest on them for the life of the loan.
              </span>
            </span>
          </label>
        </div>
            ...
            {interestIncrease > 0 && (
              <div className="mt-5 rounded-lg bg-purple-900 p-4">
                ...
                <p className="mt-1.5 text-sm text-purple-200">
                  ...
                  {formatUsd(interestIncrease)} over the life of the loan. That can still be the
                  right choice — but it should be a choice, not a surprise.
                </p>
              </div>
            )}
      ...
      <ScenarioActions
        intent="refinance"
        compareTargetId="ref-balance"
        snapshot={{
          source: "refinance_break_even",
          version: "refinance-calculator@1.0.0",
          ...
          resultSnapshot: {
            currentPaymentDollars: Math.round(result.currentPaymentCents / 100),
            newPaymentDollars: Math.round(result.newPaymentCents / 100),
            monthlyPaymentChangeDollars: Math.round(result.monthlyPaymentChangeCents / 100),
            breakEvenMonths: result.breakEvenMonths
          },
          summary: `${formatUsd(result.newPaymentCents)} new estimated payment · ${result.breakEvenMonths === null ? "no positive payment break-even" : `${result.breakEvenMonths}-month break-even`}`
        }}
      />
    </>
  );
