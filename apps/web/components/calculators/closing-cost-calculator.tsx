...
export function ClosingCostCalculator() {
  ...
  return (
    <>
      ...
      <ScenarioActions
        intent="purchase"
        compareTargetId="cc-price"
        snapshot={{
          source: "closing_cost",
          version: "closing-cost-calculator@1.0.0",
          calculationVersion: result.calculationVersion,
          inputSnapshot: {
            priceDollars: price,
            downPaymentPercent,
            closingCostsDollars: closingCosts,
            prepaidsDollars: prepaids,
            sellerCreditsDollars: sellerCredits,
            lenderCreditsDollars: lenderCredits,
            earnestMoneyDollars: earnestMoney
          },
          resultSnapshot: {
            downPaymentDollars: Math.round(result.downPaymentCents / 100),
            creditsDollars: Math.round(result.creditsCents / 100),
            estimatedCashToCloseDollars: Math.round(result.estimatedCashToCloseCents / 100)
          },
          summary: `${formatUsd(result.estimatedCashToCloseCents)} estimated cash to close · ${downPaymentPercent}% down`
        }}
      />
    </>
  );
